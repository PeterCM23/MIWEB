...
function useServer() {
  ...
  return {
    ...
    post: ({ url, body }) =>
      Http({ method: "POST", url, token, body }).then(handleResponse),
    put: ({ url, body }) =>
      Http({ method: "PUT", url, token, body }).then(handleResponse),
    patch: ({ url, body }) =>
      Http({ method: "PATCH", url, token, body }).then(handleResponse),
